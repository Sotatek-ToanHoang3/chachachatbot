...
def create_generator():
    emotions_list = ", ".join([f"{emotion['en']} ({emotion['kr']})" for emotion in _get_emotion_list()])
    template = (
        f"{PromptFactory.GENERATOR_PROMPT_BLOCK_KEY_EPISODE_AND_EMOTION_DESC}\n"
        ...
        "- Only if the user explicitly mention that they do not know how to describe their emotions or vaguely expressed their emotions (e.g., feels good/bad), "
        f"tell them that they can pick emotions from the list, and append a special token {EmotionChatbotSpecialTokens.EmotionSelect} at the end.\n"
        f"- With the special token, the user will pick one or more emotions from the list of emotions: {emotions_list}.\n"
        "- Do not mention the list of emotion words as they will be shown as GUI.\n"
        "- If locale is 'kr', use the phrase like \"화면에 표시된 감정 이름 중에서 한 개 혹은 그 이상을 <em>선택</em>하고, <em>'보내기'</em>를 눌러봐.\"\n"
        "- The user's choices will be fed as a JSON list, in the format such as [{\"key\": ...}, {\"key\":\"...\"}, ...], where 'key's contain an emotion name.\n\n"
        "[Current status of the conversation]\n"
        "- If summarizer_result exists, mention that \"Currently, you and the user seem to have identified X emotion(s): [list them here]\"."
    )
    return GeminiGenerator(base_instruction=template + PromptFactory.get_speaking_rules_block(), 
...
- After empathizing, suggest one gentle, age-appropriate idea or activity that could help them cope or feel a little better. Keep it short and realistic (e.g., taking a short break, trying a small fix, or talking with someone they trust).
...
- When the user already shared why they feel that way, acknowledge their reason and avoid repeating the same question; move forward with empathy and gentle guidance.
...
- The user may describe their emotions in an open-ended way. Otherwise, the user may optionally provide a JSON-formatted list such as '[{"key": ...}, {"key": ...}, ...],' where 'key' contains a name of emotion that the user have chosen from a list: """ + 
    ", ".join([f"{em['en']} ({em['kr']})" for em in _get_emotion_list()]) + """.
...
def str_to_result(model_output: str, params: LabelDialogueSummarizerParams) -> LabelSummarizerResult:
    try:
        ...
        if len(result.identified_emotions) > 0:
            ...
            # Don't take empathized into account for positive emotions
            if len([em for em in emotion_infos if em.reason is None or (em.empathized is False and em.is_positive is False)]) > 0:
                result.next_phase = None
                return result
            else:
                result.next_phase = "find" if len([em for em in emotion_infos if em.is_positive == False]) > 0 else "record"
                return result
        else:
            result.next_phase = None
            return result
