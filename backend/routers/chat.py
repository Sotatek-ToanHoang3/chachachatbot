...
async def _initialize_chat_session(args: ChatSessionInitializeArgs, session_id: str = Path(...)):
    if session_id in active_sessions and active_sessions[session_id] is not None:
        ...
        # Ensure the very first system turn always exposes the emotion picker UI.
        metadata = system_turn.metadata or {}
        if metadata.get("select_emotion") is not True:
            metadata = dict(metadata)
            metadata["select_emotion"] = True
            updated_system_turn = system_turn.model_copy(update={"metadata": metadata})
            # Replace the stored turn so subsequent fetches include the metadata flag.
            new_session._pop_last_turn()
            new_session._push_new_turn(updated_system_turn)
            system_turn = updated_system_turn
